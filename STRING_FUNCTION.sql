...
CREATE TABLE AGENTS
   (	
	 ...
	 AGENT_NAME CHAR(40) , 
	 WORKING_AREA CHAR(35), 
	 COMMISSION DECIMAL(10,2) DEFAULT 0.05, 
	 PHONE_NO CHAR(15), 
	 COUNTRY VARCHAR(25) 
	 );
...
SELECT SUBSTRING('MD    AMIR', 1,6) AS PARTIAL_NAME;
SELECT SUBSTRING ('MD    AMIR', 1,6) AS PARTIAL_NAME;
SELECT SUBSTRING ('MD AMIR', 0,6) AS PARTIAL_NAME;-- HERE 0 AND 1 BOTH WORKS SAME
SELECT SUBSTRING (' MD AMIR', 0,6) AS PARTIAL_NAME;-- HERE, 0 STARTS WITH THE SPACE BEFORE 'M'
SELECT SUBSTRING ('  MD AMIR', 0,6) AS PARTIAL_NAME;-- HERE, 0 AND 1 STARTS WITH THE SPACE BEFORE 'M'
SELECT SUBSTRING(' ANAND KUMAR JHA', 1,3) AS PARTIAL_NAME;
SELECT SUBSTRING('ANAND KUMAR JHA', 1,3) AS PARTIAL_NAME;
SELECT SUBSTR('MD AMIR', 1,6) AS PARTIAL_NAME;
SELECT SUBSTR('MD AMIR', 0,6) AS PARTIAL_NAME;
SELECT SUBSTR('MD AMIR', 0) AS PARTIAL_NAME;
SELECT SUBSTR('MD AMIR', 1) AS PARTIAL_NAME;
SELECT SUBSTR('MD AMIR', -4,4) AS PARTIAL_NAME;-- START FROM INDEX -7 AND GIVE NEXT 3 CHARACTERS FROM THAT INDEX
SELECT SUBSTR('MD AMIR', -3) AS PARTIAL_NAME;
...
select substring('Anand Kumar Jha',1,1) || substing('Anand Kumar Jha',7,1) as initial; -- THIS ONLY WORKS IN SNOWFLAKE(USING '||' FOR CONCAT)

select concat(substring('ANAND KUMAR JHA',1,1),substring('ANAND KUMAR JHA',7,1),substring('ANAND KUMAR JHA',13,1)) AS partial_name;-- USE THIS INSTED
