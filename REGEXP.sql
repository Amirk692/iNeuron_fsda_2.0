...
select id, 
    ...
    regexp_substr(string1, 'A\\W+(\\w+)', 1, 2,'e',2) as "RESULT2",
    regexp_substr(string1, 'A\\W+(\\w+)', 1, 3,'e',3) as "RESULT3",
    regexp_substr(string1, 'A\\W+(\\w+)', 1, 4,'e',4) as "RESULT4"
    from demo3;
...
select REGEXP_COUNT('QQQABCRTRABABABCBCD', '[AB]{2}');
select REGEXP_COUNT('QQQABCRTRABTTABCBCD', '[T]');
...
select regexp_replace( 'anandjha2309@gmail.com', '@.*\\.(com)');--same result
select regexp_replace( 'anandjha2309@gmail.com', '@.*');-- same result/better to use this
select regexp_replace( 'anandjha2309@gmail.co.in', '@.*');
...
AS date_value 
FROM (SELECT 'My DOB is 04-12-1976.' AS string) a;-- removing space and special character
